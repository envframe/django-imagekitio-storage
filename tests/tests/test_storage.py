...
class ImagekitMediaStorageTests(SimpleTestCase):
    @classmethod
    def setUpClass(cls):
        super(ImagekitMediaStorageTests, cls).setUpClass()
        cls.file_content = b'Content of file'
        cls.storage = MediaImagekitStorage(tag=TAG, resource_type='raw')
    ...
    def test_prefix_configuration(self):
        with override_settings(IMAGEKIT_STORAGE={'PREFIX': 'test/prefix'}):
    ...
    def tearDownClass(cls):
        super(ImagekitMediaStorageTests, cls).tearDownClass()
        cls.storage.delete(cls.file_name)


class ManifestImagekitStorageTests(SimpleTestCase):
    def test_manifest_is_saved_to_proper_location(self):
        storage = ManifestImagekitStorage()
...
class StaticImagekitStorageTests(SimpleTestCase):
    @classmethod
    def setUpClass(cls):
        super(StaticImagekitStorageTests, cls).setUpClass()
        cls.storage = StaticImagekitStorage(tag=get_random_name())
    ...
    @mock.patch.object(imagekit, 'upload')
    def test_upload_uses_remove_extension_for_non_raw_file(self, imagekit_upload_mock):
    ...
    @classmethod
    def tearDownClass(cls):
        super(StaticImagekitStorageTests, cls).tearDownClass()
